...
def task_list(request):
    tasks = Task.objects.filter(user_id=request.user).order_by('due_date')
...
def create_workout(request):
    ...
    return render(request, 'create_workout.html', {'tasks': task})

@login_required
def workout_list(request):
    workouts = Workout.objects.filter(user_id=request.user).order_by('-date')
...
def add_body_stats(request):
    ...
    return render(request, 'add_body_stats.html')

#list body stats
@login_required
def body_stats_list(request):
    body_stats = Bodystats.objects.filter(user_id=request.user).order_by('-recorded_date')
    return render(request, 'body_stats_list.html', {'body_stats': body_stats})

@login_required
def complete_task(request, task_id):
    task = get_object_or_404(Task, task_id=task_id, user_id=request.user)
    task.completed = True
    task.save()
    return redirect('task_list')
...
def edit_workout(request, workout_id):
    ...
    return render(request, "edit_workout.html", {"workout": workout, "tasks": tasks})
...
def edit_body_stat(request, stat_id):
    ...
    return render(request, "edit_body_stat.html", {"stat": stat})
...
def delete_body_stat(request, stat_id):
    ...
    return render(request, "delete_body_stat.html", {"stat": stat})
